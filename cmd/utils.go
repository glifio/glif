...
func fetchAgentEconFromAPI(agentAddr common.Address) (*econ.AgentFi, error) {
	...
	minersStats := make([]*econ.BaseFi, len(response.MinersTerminationStats))
	...
	principal, err := getAgentPrincipalFromAPI(agentAddr)
	if err != nil {
		return nil, err
	}
...
func getAgentPrincipalFromAPI(agentAddr common.Address) (*big.Int, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("error fetching principal. Status code: %d", resp.StatusCode)
	}
