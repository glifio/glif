...
当您从 Miner 提取资金到 Agent，以提取奖励或进行付款时，您可以使用命令：<br />
...
在借款之后，存储提供商预期支付积累的费用。你可以从 GLIF 借用 FIL 的时间长短由你决定。当你不再需要所借的 FIL 时，可以在任何时间归还给 GLIF，没有到期日，也不会收取提前还款费用。
...
手动付款可以是非常麻烦的，这就是为什么我们开发了自动驾驶功能。自动驾驶是一个服务，可以自动执行：(1) 从您 Agent 的 Miner 处提取资金，并 (2) 还款到池中。
...
需要注意的是，如果 Agent 开始积累有缺陷的扇区，它可能进入“不健康”的状态。
...
返现比例表示通过返现计划返还的利息金额比例。默认值和最大值为 10%。不过，卡片持有人可以通过以下命令设定此比例：
